...
{
	DWORD dwPatchSize;    // 得到需要patch的字节大小
	DWORD dwSaveSize = 2;	//PUSHAD，PUSHFD
	//DWORD dwOldProtect;
	LPVOID lpHookFunc;    // 分配的Hook函数的内存
	DWORD dwBytesNeed;    // 分配的Hook函数的大小
	LPBYTE lpPatchBuffer; // jmp 指令的临时缓冲区
	...
	*/
	...
	dwBytesNeed = sizeof(DWORD) + dwSaveSize + JMP_SIZE + dwSaveSize + dwPatchSize + JMP_SIZE;
	...
	*(DWORD *)lpHookFunc = dwPatchSize;
	...
	//PUSHAD，PUSHFD
	((BYTE*)lpHookFunc)[0] = 0x60;
	((BYTE*)lpHookFunc)[1] = 0x9C;

	((BYTE *)lpHookFunc + dwSaveSize + JMP_SIZE)[0] = 0x9D;
	((BYTE *)lpHookFunc + dwSaveSize + JMP_SIZE)[1] = 0x61;

	//开始backup函数开头的字
	memcpy((BYTE *)lpHookFunc + dwSaveSize + JMP_SIZE + dwSaveSize, OrgProc, dwPatchSize);
	...
	//jmp到Hook
	*((BYTE *)lpHookFunc + dwSaveSize) = 0xE9;
	*(DWORD*)((DWORD)lpHookFunc + dwSaveSize + 1) = (DWORD)NewProc - (DWORD)lpHookFunc - dwSaveSize - JMP_SIZE;

	//跳回原始
	*(BYTE *)((DWORD)lpHookFunc + dwSaveSize + JMP_SIZE + dwSaveSize + dwPatchSize) = 0xE9;
	*(DWORD*)((DWORD)lpHookFunc + dwSaveSize + JMP_SIZE + dwSaveSize + dwPatchSize + 1) = ((DWORD)OrgProc + dwPatchSize) - ((DWORD)lpHookFunc + dwSaveSize + JMP_SIZE + dwPatchSize + dwSaveSize) - JMP_SIZE;
	...
	*(DWORD*)(lpPatchBuffer + 1) = (DWORD)lpHookFunc - (DWORD)OrgProc - JMP_SIZE;
	...
	WriteReadOnlyMemory((LPBYTE)OrgProc, lpPatchBuffer, dwPatchSize);
	__free(lpPatchBuffer);
	*RealProc = (void *)((DWORD)lpHookFunc + dwSaveSize + JMP_SIZE);
